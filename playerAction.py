"""
playerAction.py  ―  GUI screen that shows team scores and a live event log.

Key points
----------
* Keeps the original scoring behaviour exactly the same.
* Adds a scroll‑able, read‑only Text widget that receives each UDP message.
* Uses a `queue.Queue()` so the background UDP thread never touches
  Tk widgets directly (Tkinter is single‑threaded).
* The UDP server thread is given:
      – score_labels, player_frames      (unchanged)
      – log_event                        (new)
      – handle_score_event, player_scores
  so it can update scores without a circular import.
"""
...
def fetch_players():
    ...
    if not conn:
        return []
    cur = conn.cursor()
    cur.execute("SELECT player_id, name, equipment_id, team FROM players ORDER BY id")
    rows = cur.fetchall()
...
def start_game(root, players=None):
    ...
    # build player_scores dict
    player_scores = {}
    for pid, name, equip, team in players:
        try:
            equip_id = int(equip)
            player_scores[equip_id] = {"codename": name, "score": 0, "team": team}
        except ValueError:
            continue
    ...
    def make_panel(bg, logo_file, team_rows):
        frame = tk.Frame(root, bg=bg, highlightthickness=2, highlightbackground="#FFFF33")
        score_lbl = tk.Label(frame, text="Score: 0", bg=bg, fg="#FFFF33", font=("Arial", 16))
        ...
        for row in team_rows:
            tk.Label(plist,
                     text=f"ID: {row[0]} | Name: {row[1]} | Equipment: {row[2]}",
                     bg=bg, fg="#FFFF33", font=("Arial", 12)
                     ).pack(anchor="w", pady=2)
        return frame, score_lbl, plist

    red_frame , red_score , red_list   = make_panel("#592020", "redTeam.png",   red_team)
    green_frame, green_score, green_list = make_panel("#20592e", "greenTeam.png", green_team)
    ...
    threading.Thread(
        ...
        args=(score_labels,
              player_frames,
              log_event,
              handle_score_event,   # pass function to avoid circular import
              player_scores),
        daemon=True
    ).start()

    # send start‑signal 202
...
# --------------------------------------------------------------------
#  handle_score_event (unchanged scoring logic)
# --------------------------------------------------------------------
def handle_score_event(player_id, team, score_label, players_frame):
    ...
    for w in players_frame.winfo_children():
        w.destroy()
    for pid, rec in sorted(player_scores.items(), key=lambda x: -x[1]["score"]):
        if rec["team"] == team:
            tk.Label(players_frame,
                     text=f"ID: {pid} | Name: {rec['codename']} | Score: {rec['score']}",
